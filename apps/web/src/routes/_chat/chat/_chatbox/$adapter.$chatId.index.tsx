...
export const Route = createFileRoute("/_chat/chat/_chatbox/$adapter/$chatId/")({
  ...
  head: ({ params, loaderData }) => ({
    meta: [
      {
        title: `Chat | ${loaderData?.chatData?.title ?? "New Chat"} | ${params.adapter.charAt(0).toUpperCase() + params.adapter.slice(1)} | Envostart`,
      },
      {
        name: "Envostart",
      ...
      {
        property: "og:title",
        content: `Chat | ${loaderData?.chatData?.title ?? "New Chat"} | ${params.adapter.charAt(0).toUpperCase() + params.adapter.slice(1)} | Envostart`,
      },
      {
